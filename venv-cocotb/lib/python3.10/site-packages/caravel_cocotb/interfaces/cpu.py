...
class RiskV:
    ...
    async def drive_data_with_address(self, address, data, SEL=0xF):
        ...
        self.cpu_hdl.dBusWishbone_ADR.value = common.signal_valueZ_size(
            self.cpu_hdl.dBusWishbone_ADR
        )[0]
        self.cpu_hdl.dBusWishbone_DAT_MOSI.value = common.signal_valueZ_size(
    ...
    async def drive_data2address(self, address, data, SEL=0xF):
        cocotb.log.info(
            f"[RiskV][drive_data2address] start driving address {hex(address)} with {hex(data)}"
        )
        ...
        if not Macros["GL"]:
            ...
            for i in range(30):
                self.cpu_hdl._id(f"dBusWishbone_ADR[{i}]", False).value = (
                    address_temp >> i
                ) & 1
        if not Macros["GL"]:
            self.cpu_hdl.dBusWishbone_DAT_MOSI.value = data
            ...
            for i in range(32):
                self.cpu_hdl._id(f"dBusWishbone_DAT_MOSI[{i}]", False).value = (
                    data >> i
                ) & 1
    ...
    async def read_address(self, address, SEL=0xF):
        ...
        await ClockCycles(self.clk, 1)
        cocotb.log.info(
            f"[RiskV][read_address] finish reading address {hex(address)} data =  {data}"
        )
        ...
        while True:
            if self.force_reset:
                ...
                if not Macros["GL"]:
                    ...
                    common.drive_hdl(
                        self.cpu_hdl.mgmtsoc_vexriscv_debug_reset, (0, 0), 1
                    )
            else:
                if first_time_release:
                    ...
                    if not Macros["GL"]:
                        ...
                        common.drive_hdl(
                            self.cpu_hdl.mgmtsoc_vexriscv_debug_reset, (0, 0), 0
                        )
